...
async function findAll(filePath) {
  ...
  try {
    const tutors = await new Promise((resolve, reject) => {
      db.all(
        'SELECT id, name, image, major, classification, subjects, availability, email FROM tutors ORDER BY id ASC',
...
async function findById(filePath, tutorId) {
  ...
  try {
    const tutor = await new Promise((resolve, reject) => {
      db.get(
        'SELECT id, name, image, major, classification, subjects, availability, email FROM tutors WHERE id = ?',
        [Number(tutorId)],
